...
| **Buzz** | Current external coordination adapter, using signed NIP-29 kind-9 channel messages. |
...
| **PeerTransport** | Transport-neutral contract that carries typed payment messages between Hermes peers. |
| **HermesPeer** | Role-neutral endpoint that composes local policy with a `PeerTransport`. |
